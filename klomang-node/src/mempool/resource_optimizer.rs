...
use crate::storage::schema::{TransactionInput, TransactionOutput, TransactionValue};
...
impl HotTier {
    ...
    pub fn add_transaction(
        ...
        self.current_memory_bytes
            .fetch_add(memory_size, std::sync::atomic::Ordering::SeqCst);
    ...
    pub fn remove_transaction(
        &self,
        tx_hash: &[u8],
    ) -> Option<(Arc<Transaction>, TransactionMetadata)> {
        if let Some((_key, (tx, metadata))) = self.transactions.remove(tx_hash) {
            self.current_memory_bytes
                .fetch_sub(metadata.memory_size, std::sync::atomic::Ordering::SeqCst);
            Some((tx, metadata))
        } else {
            None
        }
...
impl ColdTier {
    ...
    pub fn store_transaction(&self, tx: &Transaction) -> Result<(), String> {
        ...
        self.kv_store
            .put_transaction(tx_hash, &tx_value)
...
impl HybridPolicy {
    ...
    pub fn calculate_eviction_score(&self, metadata: &TransactionMetadata) -> f64 {
        ...
        let fee_component = fee_score * config.fee_weight; // Higher fee = higher score = lower eviction priority
...
impl ResourceOptimizer {
    ...
    pub fn add_transaction(
        ...
        match self
            .hot_tier
            .add_transaction(tx_hash.clone(), tx.clone(), fee_rate)
        {
            Ok(()) => Ok(()),
            Err(_) => {
    ...
    pub fn get_transaction(&self, tx_hash: &[u8]) -> Result<Option<Arc<Transaction>>, String> {
        ...
        if let Some(tx) = self.cold_tier.retrieve_transaction(tx_hash)? {
            ...
            let _ = self
                .hot_tier
                .add_transaction(tx_hash.to_vec(), tx.clone(), fee_rate);
    ...
    pub fn promote_transaction(&self, tx_hash: &[u8]) -> Result<(), String> {
        if let Some(tx) = self.cold_tier.retrieve_transaction(tx_hash)? {
            ...
            // Add to Hot Tier
            self.hot_tier
                .add_transaction(tx_hash.to_vec(), tx, fee_rate)?;
...
mod tests {
    ...
    fn test_hot_tier_add_get() {
        ...
        assert!(hot_tier
            .add_transaction(tx_hash.clone(), tx.clone(), 10)
            .is_ok());
    ...
    fn test_eviction_score_calculation() {
        ...
        let metadata = TransactionMetadata {
            arrival_time: now - 1000, // 1 second ago
            fee_rate: 20,
            last_access_time: now - 1000,
            memory_size: 1000,
