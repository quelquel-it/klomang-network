...
use crate::network::connectivity::{ConnectivityBehaviour, ConnectivityConfig};
...
pub fn initialize_network_stack() -> Result<(PeerId, JoinHandle<()>), NetworkInitializationError> {
    ...
    let connectivity_behaviour = ConnectivityBehaviour::new(
        identity.peer_id().clone(),
        ConnectivityConfig::default(),
    );

    let combined_behaviour = NodeBehaviour {
        discovery: discovery_behaviour,
        connectivity: connectivity_behaviour,
    };
...
let mut bootstrap_retry_sleep: Option<Pin<Box<tokio::time::Sleep>>> = None;
        ...
        if let Err(err) = discovery_state
            .rendezvous_control
            .register_all(&mut swarm.behaviour_mut().rendezvous)
        {
        ...
        loop {
            tokio::select! {
                ...
                _ = rendezvous_interval.tick() => {
                    discovery_state
                        .rendezvous_control
                        .discover_via_rendezvous(&mut swarm.behaviour_mut().rendezvous);
                    if let Err(err) = discovery_state
                        .rendezvous_control
                        .register_all(&mut swarm.behaviour_mut().rendezvous)
                    {
