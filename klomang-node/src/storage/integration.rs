...
use crate::storage::schema::{
    BlockValue, HeaderValue, TransactionValue, UtxoValue, 
    make_utxo_key, to_bytes, from_bytes, TransactionInput, TransactionOutput,
};
...
impl CoreIntegration {
    ...
    pub fn get_utxo(&self, outpoint: &CoreOutPoint) -> StorageResult<Option<UtxoValue>> {
        let tx_hash_bytes = to_bytes(&outpoint.0)
            .map_err(|e| StorageError::SerializationError(e.to_string()))?;
        let key = make_utxo_key(&tx_hash_bytes, outpoint.1);
        
        self.storage.cache_layer.db()
    ...
    pub fn apply_tx(&self, tx: &Transaction) -> StorageResult<()> {
        ...
        let tx_hash_bytes = to_bytes(&tx.id)
            .map_err(|e| StorageError::SerializationError(e.to_string()))?;
        ...
        self.storage.cache_layer.db().write_batch(batch)
            .map_err(|e| StorageError::DbError(e.to_string()))
    ...
    pub fn store_block(&self, block: &BlockNode) -> StorageResult<()> {
        if block.transactions.is_empty() {
            return Err(StorageError::OperationFailed("Block must have transactions".into()));
        }
        ...
        let parent_hashes: Result<Vec<_>, StorageError> = block.header.parents.iter()
        ...
        let transactions_bytes: Result<Vec<_>, StorageError> = block.transactions.iter()
        ...
        let header_bytes = to_bytes(&header_value)
            .map_err(|e| StorageError::SerializationError(e.to_string()))?;
        let block_bytes = to_bytes(&block_value)
            .map_err(|e| StorageError::SerializationError(e.to_string()))?;

        self.storage.cache_layer.db().put(ColumnFamilyName::Headers, &block_hash_bytes, &header_bytes)
            .map_err(|e| StorageError::DbError(e.to_string()))?;
        self.storage.cache_layer.db().put(ColumnFamilyName::Blocks, &block_hash_bytes, &block_bytes)
            .map_err(|e| StorageError::DbError(e.to_string()))?;

        // Store each transaction
        for tx in &block.transactions {
            let tx_id_bytes = to_bytes(&tx.id)
                .map_err(|e| StorageError::SerializationError(e.to_string()))?;
            ...
            let tx_bytes = to_bytes(&tx_value)
                .map_err(|e| StorageError::SerializationError(e.to_string()))?;
            self.storage.cache_layer.db().put(ColumnFamilyName::Transactions, &tx_id_bytes, &tx_bytes)
                .map_err(|e| StorageError::DbError(e.to_string()))?;
        }
