...
use klomang_node::storage::{KvStore, StorageCacheLayer, StorageConfig, StorageDb};

use klomang_node::storage::schema::{
    BlockValue, DagNodeValue, DagTipsValue, TransactionInput, TransactionOutput, TransactionValue,
    UtxoSpentValue, UtxoValue,
};
...
pub fn example_performance_config() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "- Block cache: {} GB",
        config.block_cache_size / (1024 * 1024 * 1024)
    );
...
pub fn example_dag_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(current_tips) = kv_store.get_current_dag_tips()? {
        println!(
            "Current DAG has {} tip blocks",
            current_tips.tip_blocks.len()
        );
    }
