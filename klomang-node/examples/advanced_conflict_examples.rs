...
pub fn example_deterministic_resolution_fee_rate() {
    ...
    conflict_map
        .register_transaction(&tx_low_fee, &low_hash)
        .ok();
...
pub fn example_dependency_chain_cascade() {
    ...
    match affected {
        Ok(affected_txs) => {
            println!(
                "✓ Conflict propagated to {} transactions:",
                affected_txs.len()
            );
            for tx in &[&prev_tx, &alice_tx, &bob_tx, &carol_tx] {
                ...
                println!(
                    "  - {:?}: {}",
                    tx,
                    if in_conflict { "CONFLICT" } else { "OK" }
                );
            }
            println!();
        }
...
pub fn example_multiple_input_conflict() {
    ...
    conflict_map
        .register_transaction(&tx_expensive, &expensive_hash)
        .ok();
...
pub fn example_orphaned_transaction_handling() {
    ...
    graph
        .mark_conflict(&parent, "Double-spend detected".to_string())
        .ok();

    println!("Impact on children:");
    println!(
        "  - Child1: {} (connected to conflicted chain)",
        if graph.is_in_conflict(&child1) {
            "ORPHANED"
        } else {
            "OK"
        }
    ...
    println!(
        "  - Child2: {} (connected to conflicted chain)",
        if graph.is_in_conflict(&child2) {
            "ORPHANED"
        } else {
            "OK"
        }
...
pub fn example_complete_network_workflow() {
    ...
    let manager = AdvancedTransactionManager::new(conflict_map, graph, pool, kv_store);
